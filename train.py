...
for epoch in range(start_epoch, epochs + 1):
    ...
    print(
        "{0}, F: {1:.2f}, MSEo: {2:.3f}, KLs: {3:.2f}, omega: {4:.2f}+-{5:.2f}, KLpi: {6:.2f}, TC: {7:.2f}, dur. {8}s".format(
            epoch, stats["F"][-1], stats["mse_o"][-1], stats["kl_div_s"][-1], stats["omega"][-1], stats["omega_std"][-1], stats["kl_div_pi"][-1], stats["TC"][-1], round(time.time() - start_time, 2)
        )
    )
    start_time = time.time()
